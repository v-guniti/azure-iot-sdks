...
**SRS_IOTHUBCLIENT_LL_02_049: [** `IoTHubClient_LL_CreateFromConnectionString` shall save `DeviceId`, `SharedAccessKey` or `SharedAccessSignature` **]**
**SRS_IOTHUBCLIENT_LL_02_050: [** If saving the `DeviceId`, `SharedAccessKey` or `SharedAccessSignature` fails then `IoTHubClient_LL_CreateFromConnectionString` shall fail and return `NULL`. **]**
**SRS_IOTHUBCLIENT_LL_02_057: [** `IoTHubClient_LL_CreateFromConnectionString` shall save the hostname from `connectionString`. **]**
**SRS_IOTHUBCLIENT_LL_02_058: [** If saving the hostname fails then `IoTHubClient_LL_CreateFromConnectionString` shall fail and return `NULL`. **]**
...
**SRS_IOTHUBCLIENT_LL_02_051: [** `IoTHubClient_LL_Create` shall save `deviceId`, `devicekey` or `deviceSasToken` from `config`. **]**
**SRS_IOTHUBCLIENT_LL_02_052: [** If saving the `deviceId`, `devicekey` or `deviceSasToken` fails then `IoTHubClient_LL_Create` shall fail and return `NULL`. **]**
**SRS_IOTHUBCLIENT_LL_02_055: [** `IoTHubClient_LL_Create` shall compute and save the IoTHub hostname from `config` by concatenating `config->iotHubName` with a '.' and `config->iotHubSuffix`. **]**
**SRS_IOTHUBCLIENT_LL_02_056: [** If hostname cannot be saved, then `IoTHubClient_LL_Create` shall fail and return `NULL`. **]**
...
**SRS_IOTHUBCLIENT_LL_02_053: [** `IoTHubClient_LL_CreateWithTransport` shall save `deviceId`, `devicekey` or `deviceSasToken` from `config`. **]**
**SRS_IOTHUBCLIENT_LL_02_054: [** If saving the `deviceId`, `devicekey` or `deviceSasToken` fails then `IoTHubClient_LL_CreateWithTransport` shall fail and return `NULL`. **]**
**SRS_IOTHUBCLIENT_LL_02_059: [** `IoTHubClient_LL_CreateWithTransport` shall get the hostname from config->protocol by calling `IoTHubTransport_GetHostName` and then it shall save it. **]**
**SRS_IOTHUBCLIENT_LL_02_060: [** If saving the hostname fails, then then `IoTHubClient_LL_CreateWithTransport` shall fail and return `NULL`. **]**
...
```
IoTHubClient_LL_UploadToBlob synchronously uploads the data pointed to by `source` having the size `size` to a blob called `destinationFileName` in Azure Blob Storage.
...
step 1: get the SasUri from IoTHub service.
...
STRING_HANDLE keyName - deviceId
If creating the HTTPAPIEX_SAS_HANDLE fails then `IoTHubClient_LL_UploadToBlob` shall fail and return `IOTHUB_CLIENT_ERROR`.
`IoTHubClient_LL_UploadToBlob` shall call `HTTPAPIEX_SAS_ExecuteRequest` passing as arguments:
